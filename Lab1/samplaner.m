...
% ta medelv�rdet av varje 2x2 område i inbilden (in) och sparar värdet som ett pixelvärd
% i den nedsamplade bilden (ut).
% Observera att inbilden (in) är tänkt att vara normaliserad mellan 0 coh
% 1.
%% Who has done it:
%
% Author: Julia Johnstone juljo166
% Co-author: Victoria Str�berg vicst257
%
...
%% Two nested for-loops
    ...
    % Looping through a matrix, at first rows then columns
    for i = 1:2:rad-1 
        for j = 1:2:kolumn-1
            
            ut(rad_counter, kolumn_counter) = double((in(i, j) + in(i+1, j) + in(i, j+1) + in(i+1,j+1))/4);
...
% images whos number of rows/columns are odd: We subtracted the rows and the columns with the value 1.
%
%